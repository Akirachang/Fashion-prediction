...
def LS(train_data, train_model, train_time=[2009, 2018]):
    all_list = []
    ...
    """
    # plt.ylabel('count')
    # plt.plot(trainY, linestyle="--", marker=".", linewidth=1.0)    # correct results
    # plt.plot(trainPredict, linestyle="--", marker=".", linewidth=1.0)  # predicted results
    # plt.show()

    return float(trainPredict.tolist()[-1][0]) / float(last_year)  # return predict result
...
def entrance(majorType, majorColor):
    ...
    main_countries = ['China', 'Thailand', 'United States of America', 'Japan', 'Korea', 'singapore', 'France', 'Germany',
                'Italy', 'England', 'Canada', 'Russia', 'Brazil', 'Mexico', 'Australia', 'India', 'Argentina', 'Spain', 'South Africa']
    if model == 'LSTM':
        country_output = []
        for country in countries:
            if country in main_countries: 
                ...
                res_predict = LS(train_data, 'year_model', train_time=[2009, 2018])
